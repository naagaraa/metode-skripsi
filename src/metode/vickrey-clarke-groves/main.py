...
class AuctionProblem:
    ...
    def __init__(self, auction, bidders):
        ...
        self._bidder_strides = np.array(
            range(nBidders), dtype=np.intp) * nCases
...
class AuctionSolver:
    ...
    def _pick_alloc(self):
        ...
        * Otherwise, an allocation is picked randomly
        ...
        old_len = len(allocs)
        allocs = [allocs[i]
                  for i, n in enumerate(total_counts) if n == min_count]
        ...
        old_len = len(allocs)
        allocs = [allocs[i]
                  for i, n in enumerate(empty_counts) if n == min_count]
        if len(allocs) < old_len:
...
# read a file .txt and parser format atau mapping data auction and bidder
