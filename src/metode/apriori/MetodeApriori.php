...
use Nagara\Src\Metode\apriori\console\Console_ProgressBar;
...
class MetodeApriori
{
   ...
   public function __construct($type = self::SRC_PLAIN, &$dataSrc = null, $minSupp = 50, $minConf = 80)
   {
      switch ($type) {
         ...
         case self::SRC_LOAD:
            ...
            break;
         default:
            throw new OutOfBoundsException('Unknown input data type ' . $type);
            break;
      }
   ...
   public function solve($minSupp = null)
   {
      ...
      for ($k = 2; isset($this->L[$k - 1]); $k++) {
         ...
         $this->L[$k] = array_filter(
            $this->C[$k], // Lk = {c ∈ Ck | c.count ≥ minsup}
   ...
   public function loadState($gzDataFile)
   {
      list(
         ...
         $this->C, $this->L, $this->rules, $this->trans,
         $this->transCount, $this->LLevels
      ) = self::loadAndPrintStateFile($gzDataFile, false);
      return $this;
   }
